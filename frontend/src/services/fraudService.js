...
const getDashboardStats = async () => {
  /*
   * Dashboard statistics are derived from backend APIs.
   * We intentionally do not return hardcoded statistics.
   */

  const [transactionsResponse, ringsResponse] =
  ...
  const transactions =
    transactionsResponse.status === 'fulfilled'
      ? asArray(transactionsResponse.value, [
          'data',
          'results',
          'transactions',
        ])
  ...
  const rings =
    ringsResponse.status === 'fulfilled'
      ? asArray(ringsResponse.value, [
          'data',
          'results',
          'rings',
        ])
  ...
  suspiciousTransactions.forEach((transaction) => {
    ...
    if (sender) highRiskWallets.add(sender);
    if (receiver) highRiskWallets.add(receiver);
  });

  const recentDetections =
    suspiciousTransactions.slice(0, 5);
  ...
  return {
    ...
    highRiskWallets: highRiskWallets.size,

    recentDetections,
    topRings: rings,

    source: 'DJANGO_REST_API',
